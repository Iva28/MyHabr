    ...
    public partial class Init : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.InsertData(
                ...
                values: new object[] { 1, "http://romanroadtrust.co.uk/wp-content/uploads/2018/01/profile-icon-png-898-300x300.png", "tikovka1@mail.ru", "tikovka1", "tikovka1", new DateTime(2018, 12, 24, 12, 43, 2, 690, DateTimeKind.Local) });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Avatar", "Email", "Login", "Password", "RegistrationDate" },
                values: new object[] { 2, "http://romanroadtrust.co.uk/wp-content/uploads/2018/01/profile-icon-png-898-300x300.png", "tikovka2@mail.ru", "tikovka2", "tikovka2", new DateTime(2018, 12, 24, 12, 43, 2, 690, DateTimeKind.Local) });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Avatar", "Email", "Login", "Password", "RegistrationDate" },
                values: new object[] { 3, "http://romanroadtrust.co.uk/wp-content/uploads/2018/01/profile-icon-png-898-300x300.png", "user@mail.ru", "user", "user", new DateTime(2018, 12, 24, 12, 43, 2, 690, DateTimeKind.Local) });
            ...
            migrationBuilder.InsertData(
                ...
                Если посмотреть определение “designated communication provider“ в параграфе 317C (пункт 6), то даже отдельный разработчик, если он гражданин Австралии, должен выполнить требование правоохранительных органов, внедрить бэкдор в программу и обязан скрыть эту информацию от своего работодателя, иначе ему грозит тюремное заключение.", "Любая интернет-компания обязана тайно изменить программный код по требованию властей", 1 },
                    { 2, new DateTime(2018, 12, 24, 12, 43, 2, 799, DateTimeKind.Local), "https://habrastorage.org/webt/go/dv/qj/godvqj9smomw8lpr4kiymtkxnzg.jpeg", @"Онбординг—это процесс, в рамках которого вы знакомите пользователя с вашим продуктом.
                ...
                3. Дать мыслям блуждать и не упустить пришедшие идеи.", "Дизайн-дайджест: онбординг, фидбек, поиск идей и принятие решений", 1 },
                    { 3, new DateTime(2018, 12, 24, 12, 43, 2, 799, DateTimeKind.Local), "https://habrastorage.org/getpro/habr/post_images/9cf/0ad/c99/9cf0adc990a16a5f4b3589254d6d1ecf.png", "Это классическая статья о том, как наша команда проверила открытый проект LibrePCB с помощью статического анализатора кода PVS-Studio. Однако статья интересна тем, что проверка осуществлялась внутри Docker контейнера. Если вы использует контейнеры, то надеемся, что статья продемонстрирует ещё один простой способ встроить анализатор в процесс разработки.", @"LibrePCB
                ...
                Значит, в сборке для современных браузеров мы можем убрать ненужные полифилы и уменьшить количество трансформаторов, что сильно скажется на размере итоговых файлов!", "Делаем Modern Build", 2 },
                    { 5, new DateTime(2018, 12, 24, 12, 43, 2, 799, DateTimeKind.Local), "https://habrastorage.org/webt/au/vq/2d/auvq2dwy-nnfmfigswo_nxrw-ae.jpeg", @"Я дизайнер интерфейсов, и когда меня бесят вещи, я рисую им новый интерфейс. Обычно это никак не влияет на проблему, но становится легче.
                ...
                Конечно, найти проблему статическим анализатором – хорошо, но найти и воспроизвести – это уже совершенно другой уровень удовольствия. :)", "Стреляем в ногу, обрабатывая входные данные", 3 }
                });
